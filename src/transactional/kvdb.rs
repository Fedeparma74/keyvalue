...
pub trait KVReadTransaction<'a>: MaybeSendSync {
    ...
    ///
    /// See the non-transactional trait for semantics.  Implementations must
    /// honour the transaction snapshot and any pending writes
    ...
    /// is set by the table, not by `range.limit`. There is no safe generic
    /// fallback, so the compiler asks for this rather than letting it pass
    /// silently.
    #[allow(clippy::type_complexity)]
    fn iter_range(
        &self,
